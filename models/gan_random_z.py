...
class gan_random_z(GAN):
    ...
    def _train_cls(self, args, epoch, disc, train_loader, optimizer_d):
        ...
        ytrue, ytrue_gen = [], []
    #     gen_loss_lst = []
        for i, (images, featmaps, targets, indexes) in enumerate(train_loader):
            images, featmaps, targets = images.to(args.device), featmaps.to(args.device), targets.to(args.device)
    ...
    def _test(self, args, epoch, disc, gen, test_loader, test_output_dir):
        ...
        for i, (inputs, featmaps, targets, indexes) in enumerate(test_loader):
            ...
            for j, target in enumerate(targets.detach().cpu().numpy().astype(int)):
                class_featmaps[target, class_idx[target]] = featmaps[j].view(256*14*14).detach().cpu().numpy()
                class_featmaps_gen[target, class_idx[target]] = gen_image[j].view(256*14*14).detach().cpu().numpy()
                class_idx[target] += 1
